...
pm_gradient = PatternMatcher([
  ...
  (UPat(Ops.INDEX, name="ret"), lambda ctx, ret:
    (ret.src[0].const_like(0).scatter_reduce(ret.arg, ret.src[1], ctx, "sum", include_self=False), None)
    if isinstance(ret.arg, int) else None),
