...
def uops_to_llvm_ir(function_name:str, uops:UOpGraph) -> str:
  ...
  for u in uops:
    if u.uop in (UOps.DEFINE_GLOBAL, UOps.DEFINE_VAR):
      if u.uop is UOps.DEFINE_GLOBAL: buf_to_dtype.update({(j, f"fake_{j}", False):PtrDType(dtypes.float) for j in range(len(buf_to_dtype),u.arg[0])})
