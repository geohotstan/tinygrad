...
class TestOps(unittest.TestCase):
  ...
  def test_sign(self):
    ...
    # helper_test_op(None, torch.sign, Tensor.sign, vals=[[[3,3,4,-5],[-3,-5,2,3]]], forward_only=True)
  ...
  def test_slice_fancy_indexing(self):
    ...
    i, j, k, o, p = [Tensor(tor.detach().numpy(), dtype=dtypes.float32, requires_grad=False) for tor in [a,b,c,d,e]]
  ...
  def test_gather(self):
    # indices cannot have gradient
    b = torch.randint(3, size=[3,4,5,1,4], requires_grad=False)
    a = Tensor(b.detach().numpy(), dtype=dtypes.float32, requires_grad=False)
