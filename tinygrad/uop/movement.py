...
mop_cleanup = PatternMatcher([
  ...
  (UPat(Ops.STACK, src=UPat(Ops.INDEX, src=(UPat.var("src"), UPat(Ops.CONST))), name="stk"),
   lambda src,stk: src if all(x.arg is None for x in stk.src) and stk.shape == src.shape
   and list(range(len(stk.src))) == [x.src[1].arg for x in stk.src] else None),
  # const INDEX into STACK is src
  (UPat(Ops.INDEX, src=(UPat(Ops.STACK, name="a"), UPat.cvar("i")), name="idx", allow_any_len=True),
   lambda a,i,idx: (a.src[i.arg] if len(idx.src) <= 2 else a.src[i.arg].index(*idx.src[2:])) if idx.arg is None else None),
  ...
  (UPat(Ops.INDEX, src=(UPat(Ops.INDEX, name="idx1", allow_any_len=True),), allow_any_len=True, name="idx2"),
   ...
   if idx1.arg is None and idx2.arg is None and all(x.shape == () for x in idx1.src[1:]+idx2.src[1:]) else None),
  ...
  (UPat(Ops.INDEX, src=(UPat(Ops.INDEX, src=(UPat.var("buf"), UPat.var("idx1_arg"))),), allow_any_len=True, name="idx2"),
   lambda buf,idx1_arg,idx2: buf.index(idx1_arg.index(*idx2.src[1:]))
   if idx2.src[0].arg is None and idx2.arg is None and len(idx1_arg.shape) == len(idx2.src[1:]) else None),
])
