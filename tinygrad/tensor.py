...
class Tensor:
  ...
  def __getitem__(self, val):
    ...
    if tensor_found: # Fancy/tensor indexing
      ...
      idx = [i[1].sign().__neg__().relu() * ret.shape[i[0]] + i[1] for i in tensor_found] # TODO first contiguous fixes torch+cpu_only CI, but it causes llvm to fail. Second one fixes llvm
