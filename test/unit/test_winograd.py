...
from tinygrad.helpers import OLD_WINO
...
class TestWinograd(unittest.TestCase):
  ...
  def test_padded_conv2d(self):
    ...
    with Context(OLD_WINO=0): expected = Tensor.conv2d(x,w,padding=1).realize()
    with Context(OLD_WINO=1): result = Tensor.conv2d(x,w,padding=1).realize()
