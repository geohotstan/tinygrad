...
class Tensor:
  ...
  def triu(self:Tensor, k:int=0, inf_mask:bool=False) -> Tensor: return self._tri(self.shape[-2], self.shape[-1], k).where(self, Tensor.zeros_like(self)) if not inf_mask else self ** self._tri(self.shape[-1], self.shape[-2], k) - 1
  def tril(self:Tensor, k:int=0, inf_mask:bool=False) -> Tensor: return self._tri(self.shape[-2], self.shape[-1], k=k+1).where(Tensor.zeros_like(self), self) if not inf_mask else self ** -(self._tri(self.shape[-1], self.shape[-2], k+1) - 1) - 1
  # ***** reduce ops *****
