...
class Tensor:
  ...
  def gather(self: Tensor, idx: Tensor, dim: int):
    ...
    idx_arange = [idx] + [Tensor.arange(idx.shape[i], dtype=dtypes.int32, requires_grad=False) for i in self_idx[:dim]] + [Tensor.arange(idx.shape[i], requires_grad=False).reshape(*[1]*(n+1), idx.shape[i]) for n,i in enumerate(self_idx[dim+1:])]
