...
class TestOps(unittest.TestCase):
  ...
  def test_slice_fancy_indexing(self):
    ...
    i, j, k, o = [Tensor(tor.detach().numpy(), requires_grad=False) for tor in [a,b,c,d]]
    helper_test_op([(2,5,15,5,3,4)], lambda x: x[a,b,c,d], lambda x: x[i,j,k,o])
    helper_test_op([(2,5,15,5,3,4)], lambda x: x[:,b,c,d], lambda x: x[:,j,k,o])
    helper_test_op([(2,5,15,5,3,4)], lambda x: x[:,b,c,:], lambda x: x[:,j,k,:])
    helper_test_op([(2,5,15,5,3,4)], lambda x: x[a,b,...], lambda x: x[i,j,...])
    helper_test_op([(2,5,15,5,3,4)], lambda x: x[a,...,d], lambda x: x[i,...,o])
    helper_test_op([(2,5,15,5,3,4)], lambda x: x[...,d], lambda x: x[...,o])
    helper_test_op([(2,5,15,5,3,4)], lambda x: x[a,:,None,d], lambda x: x[i,:,None,o])
    helper_test_op([(2,5,15,5,3,4)], lambda x: x[1,:,10:11,d], lambda x: x[1,:,10:11,o])
    helper_test_op([(2,5,15,5,3,4)], lambda x: x[1,4,c,2], lambda x: x[1,4,k,2])
