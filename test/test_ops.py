...
class TestOps(unittest.TestCase):
  ...
  def test_slice_fancy_indexing(self):
    ...
    a = torch.randint(high=1, size=(2,1,1,1,1,1), dtype=torch.int64, requires_grad=False)
    b = torch.randint(low=1, high=2, size=(1,3,1,1,1,1), dtype=torch.int64, requires_grad=False)
    c = torch.randint(high=5, size=(2,1,4,1,1,1), dtype=torch.int64, requires_grad=False)
    ...
    o = Tensor(d.detach().numpy(), requires_grad=False)
    p = Tensor(e.detach().numpy(), requires_grad=False)
    # i, j, k, o, p = [Tensor(tor.detach().numpy(), requires_grad=False) for tor in [a,b,c,d,e]]
    helper_test_op([(3,5,6,10,7)], lambda x: x[a,b,c,d,e], lambda x: x[i,j,k,o,p])
    helper_test_op([(3,5,6,10,7)], lambda x: x[:,b,c,d,e], lambda x: x[:,j,k,o,p])
    helper_test_op([(3,5,6,10,7)], lambda x: x[:,b,c,d,:], lambda x: x[:,j,k,o,:])
    helper_test_op([(3,5,6,10,7)], lambda x: x[a,b,...], lambda x: x[i,j,...])
    helper_test_op([(3,5,6,10,7)], lambda x: x[a,...,e], lambda x: x[i,...,p])
    helper_test_op([(3,5,6,10,7)], lambda x: x[...,c,:,e], lambda x: x[...,k,:,p])
    helper_test_op([(3,5,6,10,7)], lambda x: x[a,:,None,d,e], lambda x: x[i,:,None,o,p])
    helper_test_op([(3,5,6,10,7)], lambda x: x[1,:,10:11,d,0:2], lambda x: x[1,:,10:11,o,0:2])
    helper_test_op([(3,5,6,10,7)], lambda x: x[1,4,c,d,2], lambda x: x[1,4,k,o,2])
