...
class Tensor:
  ...
  def gather(self: Tensor, idx: Tensor, dim: int):
    ...
    idx_arange = [idx] + [Tensor.arange(idx.shape[i], dtype=dtypes.float32, requires_grad=False) for i in self_dim[:dim]] + [Tensor.arange(idx.shape[i], dtype=dtypes.float32, requires_grad=False).reshape(*[1]*(n+1), idx.shape[i]) for n,i in enumerate(self_dim[dim+1:])]
