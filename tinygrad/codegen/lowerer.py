...
from tinygrad.helpers import all_int, prod, partition, flatten, unwrap, QUANTIZE, LATE_FLOAT16_CAST
...
# **** push intermediate casts from f32 to f16 rightward through ALUs ****
f32_cast_f16 = UPat.var("f32", dtype=dtypes.float32).cast(dtypes.half)
f16 = UPat.any(UPat.var("f16", dtype=dtypes.half), UPat.cvar("f16", dtype=dtypes.half))
pm_push_cast = PatternMatcher([
  (UPat(GroupOp.Unary, src=(f32_cast_f16,), name="x"),
   lambda x,f32: UOp(x.op, dtypes.float32, (f32,), x.arg).cast(dtypes.half)),
  (UPat(GroupOp.Binary, src=[f32_cast_f16, f16], dtype=dtypes.float16, name="x"),
   lambda x,f32,f16: f32.alu(x.op, f16.cast(dtypes.float32)).cast(dtypes.float16)),
  # where triangle, two branches of where both originate from cast to float
  (UPat.var("cond", dtype=dtypes.bool).where(f32_cast_f16, f16), lambda cond,f32,f16:
   UOp(cond.op, cond.dtype, (cond.src[0].cast(dtypes.float32), f32)).where(f32, f16.cast(dtypes.float32)).cast(dtypes.half)),
  # fold cast
  (f32_cast_f16.cast(dtypes.float32), lambda f32: f32)
])
...
def rewrite_shapetracker_with_index(ast:UOp, opts:Renderer) -> UOp:
  ...
  if LATE_FLOAT16_CAST: ast = graph_rewrite(ast, pm_push_cast, name="push_cast_right")
