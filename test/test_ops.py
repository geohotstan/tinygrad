...
class TestOps(unittest.TestCase):
  ...
  def test_slice_fancy_indexing(self):
    ...
    helper_test_op([(2,2,6,5,3)], lambda x: x[a,b,c,d,e], lambda x: x[i,j,k,o,p])
    helper_test_op([(2,2,6,5,3)], lambda x: x[:,b,c,d,e], lambda x: x[:,j,k,o,p])
    helper_test_op([(2,2,6,5,3)], lambda x: x[:,b,c,d,:], lambda x: x[:,j,k,o,:])
    helper_test_op([(2,2,6,5,3)], lambda x: x[a,b,...], lambda x: x[i,j,...])
    helper_test_op([(2,2,6,5,3)], lambda x: x[a,...,e], lambda x: x[i,...,p])
    helper_test_op([(2,2,6,5,3)], lambda x: x[...,c,:,e], lambda x: x[...,k,:,p])
    helper_test_op([(2,2,6,5,3)], lambda x: x[a,:,None,d,e], lambda x: x[i,:,None,o,p])
    helper_test_op([(2,2,6,5,3)], lambda x: x[1,:,10:11,d,0:2], lambda x: x[1,:,10:11,o,0:2])
    helper_test_op([(2,2,6,5,3)], lambda x: x[1,4,c,d,2], lambda x: x[1,4,k,o,2])
